...
trueClassIndex(heart_scale_label==1) = 5; 
trueClassIndex(heart_scale_label==-1) = 50;
colorTrueClass = colorList(trueClassIndex,:);
% result Class
resultClassIndex = zeros(length(predict_label),1);
resultClassIndex(predict_label==1) = 5; 
resultClassIndex(predict_label==-1) = 50;
...
% The filled markers represent data instance from the test set,
% and filled color represents the class label assigned by SVM whereas
% the edge color represents the true (ground-truth) label.
% The marker size of the test set represents the probability that the
% sample instance is assigned with its corresponding class label;
